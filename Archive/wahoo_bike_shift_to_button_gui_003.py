...
# NEW: Per-button behavior — "tap" = tap on press; "hold" = hold down until release.
# (Anything not listed defaults to "tap".)
BUTTON_BEHAVIOR: Dict[str, str] = {
    "Right Steer": "hold",
    "Left Steer":  "hold",
    # Example: if you want brakes to hold too, uncomment:
    # "Right Brake": "hold",
    # "Left Brake":  "hold",
}
...
class BikeKeyGUI:
    ...
    def _build_ui(self):
        ...
        hint = ttk.Label(
            self.root,
            text="Note: key taps/holds go to the active foreground app. On macOS, allow Accessibility for Python."
        )
    ...
    async def _ble_main(self):
        ...
        while not self.stop_event.is_set():
            ...
            def on_disconnected(_client):
                # Called by Bleak from OS thread; signal the loop safely
            ...
            async def notification_handler(_char, data: bytearray):
                ...
                if not evt:
                    if self.debug_var.get():  # only show other frames when Debug output is checked
            ...
            await asyncio.sleep(RECONNECT_DELAY_S)
            # Keep 'dev' as-is; if that fails, next loop iteration will re-scan by setting dev=None
            # (Here we keep dev to speed up reconnect.)
