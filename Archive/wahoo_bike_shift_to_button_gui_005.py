...
- Per-button behavior: Tap on press OR Hold until release (prefix-first mapping)
...
# Preferred: behavior by prefix (immune to naming typos); values: "hold" or "tap".
PREFIX_BEHAVIOR: Dict[str, str] = {
    "0008": "hold",  # Right Steer
    "2000": "hold",  # Left  Steer
    # Uncomment if you want brakes to hold too:
    # "4000": "hold",  # Right Brake
    # "0100": "hold",  # Left  Brake
}

# Fallback: behavior by name (if prefix not present above). Values: "hold" or "tap".
BUTTON_BEHAVIOR: Dict[str, str] = {
    "Right Steer": "hold",
    "Left Steer":  "hold",
    # others default to "tap"
}
...
def _key_obj(key_name: str):
    return _NAME_TO_SPECIAL.get(key_name, key_name)

def send_key_tap(key_name: str):
    """Tap a key (keydown+keyup)."""
    if not key_name:
        return
    ko = _key_obj(key_name)
    keyboard.press(ko)
    keyboard.release(ko)
...
def send_key_down(key_name: str):
    ...
    if not key_name:
        return
    keyboard.press(_key_obj(key_name))

def send_key_up(key_name: str):
    """Release a previously held key."""
    if not key_name:
        return
    keyboard.release(_key_obj(key_name))
...
class BikeKeyGUI:
    ...
    def _build_ui(self):
        ...
        hint = ttk.Label(
            self.root,
            text="Keys go to the current foreground app. On macOS, allow Accessibility for Python."
        )
    ...
    async def _ble_main(self):
        ...
        while not self.stop_event.is_set():
            ...
            async def notification_handler(_char, data: bytearray):
                ...
                self.msgq.put(("log", f"[BLE] {name} {ev_type} seq={seq} ({prefix}{rr})"))

                # Resolve behavior: prefix first (robust), then name (fallback), default to "tap"
                behavior = PREFIX_BEHAVIOR.get(prefix, BUTTON_BEHAVIOR.get(name, "tap"))
                key_name = BUTTON_TO_KEY.get(name)

                # Optional debug of behavior/held-state
                if self.debug_var.get():
                    held_state = name in _HELD_BY_BUTTON
                    self.msgq.put(("log", f"[ACT] {name} {ev_type} behavior={behavior} key={key_name} held={held_state}"))
                ...
                if ev_type == "press":
                    ...
                    # if behavior == "tap": ignore releases
