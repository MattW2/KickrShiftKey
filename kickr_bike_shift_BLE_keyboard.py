...
Keyboard injection backend: pyautogui (with optional typematic repeat while held)
"""
...
# ----------------------------
# Keyboard injection via pyautogui
# ----------------------------
import pyautogui

# Make actions immediate
pyautogui.PAUSE = 0.0
# If you ever trigger the fail‑safe by moving mouse to (0, 0), you can turn it off:
pyautogui.FAILSAFE = False

# Map our logical names (e.g., 'ArrowRight') to pyautogui key strings (e.g., 'right')
KEY_NAME_MAP = {
    "ArrowUp": "up",
    "ArrowDown": "down",
    "ArrowLeft": "left",
...
def _to_pyauto_key(key_name: str) -> Optional[str]:
    """Convert our logical key name to a pyautogui key string."""
    if not key_name:
        return None
    mapped = KEY_NAME_MAP.get(key_name)
    if mapped:
        return mapped
    # For single printable characters ('i', 'k', '3', etc.) pyautogui accepts the char directly.
    return key_name

def send_key_tap(key_name: str):
    k = _to_pyauto_key(key_name)
    if not k:
        return
    pyautogui.press(k)

def send_key_down_raw(k: str):
    pyautogui.keyDown(k)

def send_key_up_raw(k: str):
    pyautogui.keyUp(k)
...
# ----------------------------
# OPTIONAL: typematic repeat while held
# ----------------------------
# Some apps treat a key as "held" only if they continuously see KEYDOWN events.
# We emit periodic keyDown calls until release. Tweak REPEAT_HZ as needed.

REPEAT_HZ = 30.0           # typematic repeat frequency while held
REPEAT_DT = 1.0 / REPEAT_HZ

class HoldRepeater:
    def __init__(self):
        self._threads: Dict[str, threading.Event] = {}  # button -> stop event
        self._lock = threading.Lock()

    def start(self, button_name: str, pyauto_key: str):
        with self._lock:
            if button_name in self._threads:
                return
            stop = threading.Event()
            self._threads[button_name] = stop

        # initial keyDown
        send_key_down_raw(pyauto_key)

        # repeat loop
        def _loop():
            try:
                while not stop.wait(REPEAT_DT):
                    # Emit another keyDown to simulate typematic (no keyUp)
                    send_key_down_raw(pyauto_key)
            finally:
                pass
    ...
    def stop(self, button_name: str, pyauto_key: str):
        with self._lock:
            stop = self._threads.pop(button_name, None)
        if stop:
            stop.set()
        # final release
        send_key_up_raw(pyauto_key)

_hold_repeater = HoldRepeater()

# Track currently held keys per button so we release correctly and don't duplicate
_HELD_BY_BUTTON: Dict[str, str] = {}  # button_name -> pyautogui_key
...
def hold_if_needed(button_name: str, key_name: str):
    ...
    if button_name not in _HELD_BY_BUTTON:
        _HELD_BY_BUTTON[button_name] = k
        _hold_repeater.start(button_name, k)

def release_if_held(button_name: str):
    k = _HELD_BY_BUTTON.pop(button_name, None)
    if k:
        _hold_repeater.stop(button_name, k)

def release_all_held_keys():
    for btn, k in list(_HELD_BY_BUTTON.items()):
        try:
            _hold_repeater.stop(btn, k)
        except Exception:
            pass
...
BUTTON_BEHAVIOR: Dict[str, str] = {
    "Right Steer": "hold",
    "Left Steer":  "hold",
    # If you want brakes to be hold, uncomment:
    # "Right Brake": "hold",
    # "Left Brake":  "hold",
}
...
class BikeKeyGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("KICKR BIKE SHIFT — BLE to Keyboard")
    ...
    def _build_ui(self):
        ...
        hint = ttk.Label(
            self.root,
            text="Using pyautogui for tap/hold. Some apps need typematic repeat; enabled by default for 'hold'."
        )
    ...
    async def _ble_main(self):
        ...
        while not self.stop_event.is_set():
            ...
            # Disconnect callback (registered in ctor)
            ...
            try:
                # Connect (register disconnected callback via constructor)
                ...
                if not client.is_connected:
                    ...
                    self.msgq.put(("status", ("Error", "red")))
                    # if it fails immediately, try to re-scan & reconnect unless user stopped
                    dev = None
